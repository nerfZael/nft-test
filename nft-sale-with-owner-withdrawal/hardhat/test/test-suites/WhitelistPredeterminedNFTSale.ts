...
describe("Whitelist Specific NFT Sale", () => {
  ...
  it("can withdraw payments made", async () => {
    let erc20TokenContract = await (new MockERC20__factory(owner)).deploy("Test", "T");
    ...
    erc20TokenContract = erc20TokenContract.connect(owner);

    const ownerERC20Balance = await erc20TokenContract.balanceOf(await owner.getAddress());

    saleContract = saleContract.connect(owner);
    await saleContract.withdraw(await owner.getAddress(), 0, [erc20TokenContract.address], [price]);

    const ownerNewERC20Balance = await erc20TokenContract.balanceOf(await owner.getAddress());

    expect(ownerNewERC20Balance).to.equal(ownerERC20Balance.add(price));
  });
});
